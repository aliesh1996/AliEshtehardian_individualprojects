...
//            x3+=H(pi_c[generation])-pi_c[generation]*H(population[j]->p[0])-(1-pi_c[generation])*H(population[j]->p[1]);

            // cout<<population[j]->p[0]<<endl;
...
//            x5+=population[j]->pi[0]/(population[j]->pi[0]+population[j]->pi[1]);
        ...
        //make selection and next population (Using pariwise comparison selection method)
        nextGen.clear();
        for(int i=0;i<popSize;i++){
            ...
            //Here is when Strength of the Selection matters
...
//Here, we just recored our data for every generation (not all of them are needed necessarily)
...
//Here, we are recording our time
